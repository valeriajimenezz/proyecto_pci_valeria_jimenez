...
def menu():
    indicadores = None
    factores = {"electricidad": 0.0, "combustibles": 0.0, "residuos": 0.0}
    soluciones_base = [
        {"codigo": "paneles_solares",        "nombre": "Paneles solares",        "tipo": "energia",
            "%energia": 0.30, "capex_mxn": 300000, "meses": 6,  "dificultad": 4, "seguridad": 4},
        {"codigo": "iluminacion_led",        "nombre": "Iluminación LED",        "tipo": "energia",
...
while True:
    print("\n=== Menú Principal ===")
    print("1. Recepción de datos")
    print("2. Procesamiento de datos")
    print("3. Soluciones base:")
    print("4. Estimación económica")
    print("5. Priorizar soluciones")
    print("0. Salir")
    opcion = int(input("Seleccione una opción: "))
    if opcion == 1:
        indicadores = entrada_de_datos()
    elif opcion == 2:
        if indicadores:
            print("\n--- Métricas ---")
            print(f"- kWh por empleado: {kwh_por_empleado(indicadores):.2f}")
            print(f"- Tasa de incidentes: {tasa_incidentes(indicadores):.2f}%")
            print(
                f"- Residuos por empleado: {residuos_por_empleado(indicadores):.2f} kg")
    elif opcion == 3:
        print("\n Soluciones base:")

        for sol in soluciones_base:
            print(f"- {sol['nombre']} (Tipo: {sol['tipo']})")
        # sol = soluciones_base == [ {"codigo": "paneles_solares", "nombre": "Paneles solares", "tipo": "energia",
        #    "%energia": 0.30, "capex_mxn": 300000, "meses": 6, "dificultad": 4, "seguridad": 4},
        ...
        estimar_impacto_y_economia(sol, indicadores, 1.5, factores)
        break
    elif opcion == 5:
        print("\n Priorizar soluciones")
        ranking = sorted(
            soluciones_base, key=lambda x: x["impacto"]/x["costo"], reverse=True)
        for i, sol in enumerate(ranking, 1):
            print(
                f"{i}. {sol['nombre']} (Score: {sol['impacto']/sol['costo']:.6f})")
        break
    elif opcion == 0:
